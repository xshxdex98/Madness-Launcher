...
RACE_NAMES in madness_launcher/records/reader.py. Repeat per race; each run
...
from madness_launcher.records import reader  # noqa: E402
...
def from_json(raw: str) -> dict:
    ...
    for key, (driver, car, seconds) in json.loads(raw).items():
        ...
        out[(city, difficulty, int(slot))] = reader.LapRecord(
...
def main(argv: list[str]) -> int:
    ...
    files = reader.record_files(install)
    if not files:
        sys.exit(f"No players/*/*.dat under {install}. Has the game been run?")

    if command == "snapshot":
        current = reader.snapshot(install)
    ...
    if command == "diff":
        ...
        after = reader.snapshot(install)
        changed = reader.improvements(before, after)
        if not changed:
        ...
        if len(races) == 1 and name:
            print("Add this to RACE_NAMES in madness_launcher/records/reader.py:")
